...
function processValueChanged(variables, oldVariable, newVariable) {
    ...
    if (newVariable && oldVariable) {
        //countdown_to_zero
        if (newVariable.value === 0 && oldVariable.value > 0) {
        ...
        if (newVariable.value !== oldVariable.value && newVariable.value !== -1) {
            getTrigger('countdown_reaches_number').trigger(null, {
        ...
        if (newVariable.value !== -1 && oldVariable.value === -1) {
            ...
            getTrigger('countdown_started_updated').trigger(null, {
                "variable": newVariable.name,
                "value": newVariable.value
            });
        ...
        if (Math.round(newVariable.value) !== Math.round(oldVariable.value) && newVariable.value !== -1) {
            //console.log('variablemanager: counttimer changed')
            getTrigger('countdown_timer_changed').trigger(null, {
                "variable": newVariable.name,
                "value": newVariable.value
            });
        ...
        //countdown_timer_changed_precise
        if (newVariable.value !== oldVariable.value && newVariable.value !== -1) {
            // console.log('variablemanager: countdown_timer_changed_precise changed, new value "' + newVariable.value + '"')
            getTrigger('countdown_timer_changed_precise').trigger(null, {
                "variable": newVariable.name,
                "value": newVariable.value
            });
            //   homey.flow.getTriggerCard('countdown_timer_changed').trigger(null, { "variable": newVariable.name, "value": newVariable.value });
        ...
        //countdown_stopped
        if (newVariable.value === -1 && oldVariable.value !== 0 && oldVariable.value !== -1) {
            //console.log (newVariable)
